...
KEYSTAT		EQU     $00DE0004	
KEYIN		EQU     $00DE0000	;Console input port. Normally the Propeller Driven S-100 Console-IO Board
KEYOUT		EQU     $00DE0000	;Console output port. Normally the Propeller Driven S-100 Console-IO Board
...
IDE_Reset_Delay	EQU     $20	;Time delay for reset/initilization (~66 uS, with 8MHz 8086, 1 I/O wait state)
...
MAXSEC		EQU      $3D	;Sectors per track for CF my Memory drive, Kingston CF 8G. (CPM format, 0-3CH)
...
DOS_MAXSEC	 	EQU      $3F	;For MS-DOS BIOS Setting "Hard Disk" to Custom type (CF Card, 63 Sectors/track)
...
;------------------ START --------------------------------------------------------------------------------
   ...
   IFNE ROM_CODE		;If ROM_CODE = 1 (Use with ROM)
	...
	DC.L	$00098100	;SSP = guest RAM base
	DC.L	$00090020	;PC = guest entry point

	ORG	$00090020	;Will start monitor here in guest image
   ENDC
...
;	BSR	SPEAK_STRING	;Speak out signon the message
	...
	MOVEA.L #BeginRAM,A2	;START OF WORK RAM (PAST STACK)
...
JMP_Z80:			;Switch back to Z80 Master CPU 
	MOVEA.L	#SW68K,A2	;Adapted for vasm compatibility
...
SPINUP:	MOVE.B	#COMMANDspinup,D4
spup2:	MOVE.B	#REGcommand,D5
	BSR	IDEwr8D
	BSR	IDEwaitnotbusy
	BGE	L_7	
	BRA	SHOWerrors
L_7:	CLR.B	D1	;Clear carry
	BSR	CRLF	
	BRA	IDE_LOOP	;Back to IDE Menu
...
WR_LBA:				;numbers etc).
	...
	MOVE.B	D4,RAM_DRIVE_SEC		;For Diagnostic Diaplay Only
	MOVE.B	#REGsector,D5		;Send info to drive
	BSR	IDEwr8D		;Write to 8255 A Register
				;Note: For drive we will have 0 - MAXSEC sectors only
					
	MOVE.B	RAM_TRK,D4
	MOVE.B	D4,RAM_DRIVE_TRK		;Send Low TRK#			
	MOVE.B	#REGcylinderLSB,D5
	BSR	IDEwr8D		;Write to 8255 A Register
	
	MOVE.B	RAM_TRK+1,D4
	MOVE.B	D4,RAM_DRIVE_TRK+1
...
DOS_WR_LBA:				;instead of LBA sector numbers.
	...
	ANDI.B	#$0F,D4		;top two LED HEX displays.
...
PUTWORD_D6:	MOVE.L	D0,-(A7)	;Use D0 as scratch, preserve caller state
...
PUTCHAR0:	MOVE.B	(A0),D5	;Check UART status is ready to receive character
	ANDI.B	#$01,D5
	TST.B	D5
	BEQ	PUTCHAR0
	MOVE.B  D1,(A1)	;Output ASCII (in D1) to UART data port
...
GETCHAR:
  ...
  ENDC
   ...
   IFEQ SIMMULATOR		;If SIMMULATOR = 0, then echod character via PUTCHAR 
	MOVE.L	D5,-(A7)	;> Save D5
GETCHAR0:	MOVE.B	(A0),D5	;Get a keyboard character in D1
	ANDI.B	#$02,D5
...
IDE_TABLE		dc.l    SET_DRIVE_A		; "A"  Select Drive A
		...
		dc.l    N_RD_SEC		; "V"  Read N sectors
		dc.l    WRITE_SEC		; "W"  Write data buffer to current sector
		dc.l    N_WR_SEC		; "X"  Write N sectors
		dc.l    COPY_AB		; "Y"  Copy Drive A to Drive B
		dc.l    VERIFY_AB		; "Z"  Verify Drive A:= Drive B:
...
Signon		dc.b   CR,LF,'68000 Monitor V2.7b4 John Monahan 19/07/2020',CR,LF,0
Prompt		dc.b   CR,LF,'>',0		
Menu		dc.b   CR,LF
...
SMSG		dc.b   'THE 68 THOUSAND ROM MONITOR, VERSION 2.5 IS NOW ACTIVE',0
...
ECHO_MSG		dc.b   CR,LF,'Will echo each ASCII character typed on keyboard. ESC to abort',CR,LF,0
PATCH_MSG    	dc.b   CR,LF,'Moving Code at 4000H-9000H to F4000H, Then jump to that location',CR,LF,0
SERIAL_TEST_MSG	dc.b   CR,LF,'Characters entered via serial IN port will appear on serial OUT port.'
...
IDE_SIGNON1		dc.b   'A=Select Drive A  B=Select Drive B  C=Boot CPM68K D=Set Sec Display Mode to On',CR,LF,0
IDE_SIGNON2		dc.b   'A=Select Drive A  B=Select Drive B  C=Boot CPM68K D=Set Sec Display Mode to Off',CR,LF,0
IDE_SIGNON3		dc.b   'E=Clear Sec Buff  F=Format Disk     I=Next Sec    J=Previous Sec',CR,LF
		dc.b   'L=Set LBA Value   N=Power Down      O=Disk ID',CR,LF
		dc.b   'R=Read Sector     S=Seq Sec Rd      U=Power Up    V=Read N Sectors',CR,LF
...
INIT_1_ERROR	dc.b   CR,LF,'Initilizing of First Drive failed. Aborting Command.',BELL,CR,LF,LF,0
INIT_2_ERROR	dc.b   CR,LF,'Initilizing of Second Drive failed. (Possibly not present).',BELL,CR,LF,LF,0
...
MODEM_S_SIGNON	dc.b   CR,LF,'Load Motorola .s File from a PC to RAM using the S100Computers IO Board',CR,LF
		dc.b   'Zilog SCC Ports A1H & A3H. Requires RTS & CTS, 38,400 Baud.',CR,LF,0

MODEM_BIN_SIGNON	dc.b   CR,LF,'Load a .bin File from a PC to RAM using the S100Computers IO Board',CR,LF
		dc.b   'Zilog SCC Ports A1H & A3H. Requires RTS & CTS, 38,400 Baud.',CR,LF,0
...
;---------------------------------------------------------------------------------------------------------
   IFNE ROM_CODE		;If ROM based Code
	ORG	$000980F0	;Guest supervisor stack area
SUPERVISOR_STACK 
	DC.W	0
	ORG	$00098100	;Guest RAM area
   ENDC
